...
modified slightly for the present work
"""
...
# Set the main directory to the directory where this script is located
main_directory = os.path.dirname(os.path.abspath(__file__))

# Enable logging for debugging (optional)
logging.basicConfig(format='%(asctime)s:%(levelname)s:%(message)s', level=logging.INFO)

# Walk through each subdirectory and find all .hpl files that start with 'Stare_122'
for root, dirs, files in os.walk(main_directory):
    for file in files:
        if file.startswith('Stare_122') and file.endswith('.hpl'):
            filepath = os.path.join(root, file)
            output_directory = root  # Save output in the same subdirectory

            # Predict output NetCDF filename without processing
            try:
                with open(filepath) as f:
                    header_lines = [next(f) for _ in range(11)]  # <-- FIXED LINE
                header = decode_header(header_lines)
                start_time = datetime.strptime(header['Start time'], '%Y%m%d %H:%M:%S.%f')
                scan_type = lookup.get(header['Scan type'], 'unknown')
                output_filename = start_time.strftime(f"{scan_type}_%Y%m%d_%H%M%S.nc")
                output_filepath = os.path.join(output_directory, output_filename)
            except Exception as e:
                logging.error(f"Failed to read header from {filepath}: {e}")
                continue

            # Skip if NetCDF already exists
            if os.path.exists(output_filepath):
                logging.info(f"NetCDF already exists, skipping: {output_filepath}")
                continue

            # Process if NetCDF doesn't exist
            logging.info(f'Processing file {filepath}')
            try:
                process_file(filepath, output_directory, prefix=None)
            except Exception as e:
